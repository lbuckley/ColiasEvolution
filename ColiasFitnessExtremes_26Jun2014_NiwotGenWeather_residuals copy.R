...
#switch to median rather than mean due to skewed variances
...
clim.maxmean= aggregate(clim$Max, by=list(clim$Year), FUN=median, na.action = na.omit)
...
clim.minmean= aggregate(clim$Min, by=list(clim$Year), FUN=median, na.action = na.omit)
...
plot(clim.stat$MinVar, clim.stat$MaxVar)
#Well correlated

#----------------------
#Estimate residuals 
#Variance in 1988 is representative
#Use 1964 with high variance

clim1988= subset(clim, clim$Year== 1964)
clim1988$resid.max= clim1988$Max- clim.stat[which(clim.stat$Year==1964),"MaxMean"]
clim1988$resid.min= clim1988$Min- clim.stat[which(clim.stat$Year==1964),"MinMean"]
#plot residuals
plot(clim1988$resid.max, type="l")
points(clim1988$resid.min, col="red", type="l")

#--------------------
#Simulate using residuals
...
clim$Max.Gen[inds]= clim1988$resid.max + max.yr(yr)
clim$Min.Gen[inds]= clim1988$resid.min + min.yr(yr)
}
...
plot(clim$Min, clim.genmean$Min)
...
for(yr.k in 40:length(years)){ #loop years
