...
ELEVATION[station]
LOCATION[station]
# Monthly climate (given by observed time series)
PREC_CLIMATE <- NULL
# Calibration period
year_max <- 1990
year_min <- 1961
...
# The same of calibaration period
#Number of GPCA iterations for variable and VAR residuals:
n_GPCA_iter <- 5
n_GPCA_iteration_residuals <- 5

#Two different auto-regression orders p and no exogenous variables are considered: 
p_test <- 1
p_prec <- 3
exogen <- NULL
exogen_sim <- exogen

#Launching a Precipitation Generation with auto-regression order p = 3 and PCA Gaussianization (P03GPCA)
...
#--------------------------
#Daily temperature is modeled using precipitation data as exogenous predictors (1)

# Monthly climate (given by observed time series)
TN_CLIMATE <- NULL
TX_CLIMATE <- NULL

#The same number of GPCA iterations for variable and VAR residuals is considered:
n_GPCA_iter <- 5
n_GPCA_iteration_residuals <- 5

#Two different auto-regression orders p are considered:
p_test <- 1
p_temp <- 10

###FIND prec_mes, prec_gen

#Observed and P03GPCA generated marginally Gaussianized precipitation are taken as an exogenous variable for temperature generation:
...
#Launching a Temperature Generation with auto-regression order p = 10 and PCA Gaussianization (P10GPCA)
generationP10GPCA temp <- ComprehensiveTemperatureGenerator (station=station, Tx_all=TEMPERATURE_MAX,Tn_all=TEMPERATURE_MIN, year_min=year_min, year_max=year_max,p=p_temp, n_GPCA_iteration=n_GPCA_iter, n_GPCA_iteration_residuals=n_GPCA_iteration_residuals, exogen=exogen,exogen_sim=exogen_sim,sample="monthly", mean_climate_Tn=TN_CLIMATE,mean_climate_Tx=TX_CLIMATE)
...
if(stat.k==1)dat=a1.clim
if(stat.k==2)dat=b1.clim
...
write.csv(dat.all_20cm, "LambdaLong_20cm_shade_Niwot3June.csv", row.names=FALSE)
write.csv(dat.all_50cm, "LambdaLong_50cm_shade_Niwot3June.csv", row.names=FALSE)

#write.csv(Lambda[,2,,1], "LambdaMat.csv")

c1dat=subset(dat.all_20cm, dat.all$Station== "C1")
par(mfrow=c(1,2))
